...
class AVLTree:
    ...
    def insert(self, root, key):
        """Insert a key into the AVL tree and rebalance if needed."""
        if not root:
            return AVLNode(key)
        ...
        if balance > 1 and key < root.left.key:
            print(f"Case #3a: adding a node to an outside subtree (key={key})")
        ...
        # Case 3b: Left Right
        if balance > 1 and key > root.left.key:
            print(f"Case #3b not supported (key={key})")
